...
document.getElementById("select-sido").addEventListener("change", function () {
    ...
    let gugunUrl = `${getContextPath()}/trip?action=getGugun&sidoCode=${
        this.value
    }`;
...
document.getElementById("btn-search").addEventListener("click", () => {
    ...
    let query = document.getElementById("search-keyword").value;
    let contentType = document.querySelector(
        ".search-content-class:checked"
    ).value;
    if (!query) query = "";

    let searchUrl = `${getContextPath()}/trip?action=getAttraction&sidoCode=${sidoCode}&gugunCode=${gugunCode}&query=${query}&contentType=${contentType}`;
    if (sidoCode == 0) {
...
function displayMarker(positions) {
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        kakao.maps.event.addListener(marker, "click", function () {
            moveCenter(positions[i].contentId, positions[i].mapy, positions[i].mapx, positions[i].title, positions[i].addr1, positions[i].addr2, positions[i].image, positions[i].contentTypeId);
        });
...
function addSchedule(contentId) {
    let url = `${getContextPath()}/trip?action=addSchedule&contentId=${contentId}`;
